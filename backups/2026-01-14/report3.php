...
if (kontrola_prihlaseni() == "OK")
{
    if (isset($_SESSION["typ"]))
    {
        if ($_SESSION["typ"] == "5")
        {
            ...
            $dotaz = "SELECT DISTINCT(zamestnanci.cilova) from dochazka left join firmy on dochazka.firma = firmy.id left join zamestnanci on dochazka.zamestnanec = zamestnanci.id where datum between '" . $start_day . "' and '" . $end_day . "' and zamestnanci.cilova<>'' AND zamestnanci.os_cislo REGEXP '^[0-9]+$' order by zamestnanci.cilova";
            ...
            while ($radek = mysqli_fetch_array($vysledek))
            {   
                array_push($num_id, $radek["cilova"]);
            ...
            echo "<tr class='horizontal-line-dole'><th scope='col' class='text-center'>Cílová stanice</th><th>Obj.</th>";
            ...
            foreach ($num_id as $value) 
            {
                ...
                echo "<tr>";
                echo "<td class='table-warning text-start'>" . $value . "</td>";
                ...
                for ($x = 0; $x <= $dnu; $x++) 
                {
                    ...
                    $pocet = pocet_zam_cilovou_stanici_den($value,$datum);
