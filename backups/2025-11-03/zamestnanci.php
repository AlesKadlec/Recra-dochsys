...
{   
    if (isset($_GET["typ"]))
    {
        ...
        {          
            ...
            // --- nastavení hodnot smena / nepritomnost / smena2 dle aktivni ---
            if ((int)$_POST['aktivni'] === 0) {
                $smena_val = 'N/A';
                $nepritomnost_val = '';
                $smena2_val = 'N/A';
                $dpn_od_val = '0000-00-00'; // nepoužíváme dpn_od zde
            } else {
                // aktivni != 0
            ...
            //echo "<br><br><br>";
            //echo "checkbox_vystup:*" . $_POST["checkbox_vystup"] . "*";
            //echo $sql;
        ...
        {   
            ...
            <?php
                ...
                // Na začátku zamestnanci.php nebo před voláním filtru
                if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cilova'])) {
                    $_SESSION['filtry']['cilova'] = $_POST['cilova'];
                    $_SESSION['filtry']['pomer'] = $_POST['pomer'] ?? '1';
                    $_SESSION['filtry']['smena'] = $_POST['smena'] ?? 'R';
                    $_SESSION['filtry']['nepritomnost'] = $_POST['nepritomnost'] ?? '';
                }
            ...
            <thead>
                <!-- první řádek: hlavička pro řazení -->
                <tr class='table-active'>
                    <?php
                ...
                <!-- druhý řádek: textová pole pro filtrování -->
                <tr>
                    <?php
                    $cols = [];
                    if (($_POST['pomer'] == 0) or ($_POST['pomer'] == 33) or ($_POST['pomer'] == 44) or ($_POST['pomer'] == 55)) {
                    ...
                    foreach($cols as $col) {
                        echo "<th><input type='text' class='form-control form-control-sm' placeholder='Hledat $col'></th>";
                    }

                    if (($_SESSION['typ'] == 1) or ($_SESSION['typ'] == 4) or ($_SESSION['typ'] == 5)) {
                        echo "<th></th>"; // Editace nefiltrujeme
                    }
                    ?>
                </tr>
            </thead>
            ...
            <?php
                ...
                $_SESSION['filtry'] = [
                    ...
                    'pomer'         => $_POST['pomer'] ?? '',
                    'smena'         => $_POST['smena'] ?? '',
                    'nepritomnost'  => $nepritomnost ?? 'ALL'
                ];

                // --- načtení z SESSION pokud POST není nastaveno ---
                $cilova = $_POST['cilova'] ?? ($_SESSION['filtry']['cilova'] ?? 'ALL');
                $pomer = $_POST['pomer'] ?? ($_SESSION['filtry']['pomer'] ?? '');
                $smena = $_POST['smena'] ?? ($_SESSION['filtry']['smena'] ?? '');
                $nepritomnost = $_POST['nepritomnost'] ?? ($_SESSION['filtry']['nepritomnost'] ?? 'ALL');
                ...
                if ($_POST['cilova'] != "ALL") {
                    $cilova = mysqli_real_escape_string($conn, $_POST['cilova']);
                    $sql .= " AND zamestnanci.cilova = '$cilova'";
                } elseif ($_SESSION['typ'] != 5) {
                    // jen cílové stanice z SESSION pro ostatní typy
                ...
                // --- filtr pomeru / aktivni / radneukoncen / anulace ---
                $pomer = $_POST['pomer'] ?? '';
                switch ($pomer) {
                    case "1":
                        $sql .= " AND zamestnanci.aktivni='1'";
                        break;
                    case "0":
                        $sql .= " AND zamestnanci.aktivni='0' AND anulace<>'0000-00-00'";
                        break;
                    case "33":
                        $sql .= " AND zamestnanci.aktivni='0' AND radneukoncen='ANO'";
                        break;
                    case "44":
                        $sql .= " AND zamestnanci.aktivni='0' AND radneukoncen='NE'";
                        break;
                    case "55":
                        $sql .= " AND zamestnanci.aktivni='0' AND anulace<>'0000-00-00'";
                        break;
                    ...
                    default:
                        if ($pomer !== '' && $pomer != '0') {
                            $sql .= " AND zamestnanci.aktivni='" . intval($pomer) . "'";
                        }
                }
                ...
                if (!empty($nepritomnost) && $nepritomnost != 'ALL') {
                    if ($nepritomnost == 'Vše') {
                        // Všechny typy nepřítomností kromě prázdné (Přítomní)
                        $sql .= " AND nepritomnost IN ('DPN','OČR','DOV','ABS','NAR','LEK','NEM','NEO','NEP','PRO')";
                    } else {
                        $sql .= " AND nepritomnost = '$nepritomnost'";
                    }
                }
                    ...
                    ?>
                        ...
                        {   ?>
                                ...
                                <a><button type="button" class="btn btn-sm align-middle bg-light" data-bs-toggle='modal' data-bs-target='#ModalPrihlasitVedoucim<?php echo $radek['id'];?>' onclick="loadModalContent('<?php echo $radek['id']; ?>')">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="currentColor" class="bi bi-person-vcard" viewBox="0 0 16 16">
    ...
    {   ?>
        ...
        {   
            ...
            {   ?>
                <div class="d-grid gap-2 d-md-flex justify-content-md-center">    
                    ...
                    <a class="btn btn-outline-primary text-center m-2" href="zamestnanci.php?typ=zmenasmennosti3" role="button">Změna směnnosti<br>(následující týden) s formulářem</a>
        ...
        <thead>
            <tr class='table-active'>
                ...
                <th>Cílová st.</th>
                <th>Docházka</th>
                <?php if (($_SESSION['typ'] == 1) or ($_SESSION['typ'] == 4) or ($_SESSION['typ'] == 5)) { ?>
                    <th>Editace</th>
                <?php } ?>
            </tr>
            <!-- ✅ druhý řádek s inputy -->
            <tr>
                <th><input type="text" class="form-control form-control-sm" placeholder="Hledat ID"></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="Hledat příjmení"></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="Hledat jméno"></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="Os.č."></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="Os.č.kl."></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="RFID"></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="Telefon"></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="Vstup"></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="Výstup"></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="Doprava"></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="Nástup"></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="Směna"></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="Cílová st."></th>
                <th><input type="text" class="form-control form-control-sm" placeholder="Docházka"></th>
                <?php if (($_SESSION['typ'] == 1) or ($_SESSION['typ'] == 4) or ($_SESSION['typ'] == 5)) { ?>
                    <th></th> <!-- Editace nefiltrujeme -->
                <?php } ?>
            </tr>
        </thead>
