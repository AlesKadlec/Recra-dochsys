                    ...
                    <div class="modal-body">
                        <form name="kalendar" method="POST" action="report4.php?typ=savechange">
                            ...
                            while ($current <= $last_day_of_month) {
                                ...
                                // --- Načtení výchozí hodnoty týdne z databáze ---
                                $vychozi_smena = '';
                                $stmt = $conn->prepare("SELECT smena FROM plan_smen WHERE rok = ? AND tyden = ? AND jmeno = ?");
                                $stmt->bind_param("iis", $year, $weekNumber, $id_zam);
                                $stmt->execute();
                                $stmt->bind_result($vychozi_smena);
                                $stmt->fetch();
                                $stmt->close();

                                // 1) buňka s číslem týdne + výběrem směny + hidden last
...
function kontrola_naboru($jmeno, $prijmeni, $id = null) 
{
    global $conn;
    ...
    if ($id) {
        $stmt = $conn->prepare("SELECT COUNT(*) AS pocet FROM nabory WHERE jmeno=? AND prijmeni=? AND id<>?");
        $stmt->bind_param("ssi", $jmeno, $prijmeni, $id);
    } else {
        $stmt = $conn->prepare("SELECT COUNT(*) AS pocet FROM nabory WHERE jmeno=? AND prijmeni=?");
