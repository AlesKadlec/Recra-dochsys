...
if (isset($_GET['akce']) && $_GET['akce'] === 'kontrola_duplicit') {
    ...
    if (kontrola_naboru($jmeno, $prijmeni, $id)) {
        echo "duplicita";
    } else {
        echo "ok";
    }
...
if (kontrola_prihlaseni() == "OK") {
    ...
    if (isset($_GET["typ"])) {
        ...
        function kontrolaDuplicit() {
            ...
            let id = $("#id").val(); // pro editaci; může být prázdné

            if (jmeno.length < 2 || prijmeni.length < 2) {
                $("#dupInfo").hide();
                $("#btnUlozit").prop("disabled", false); // povolit tlačítko
                return;
            }
                ...
                data: {
                    ...
                    success: function(resp) {
                        ...
                        if (resp.trim() === "duplicita") {
                            ...
                            $("#jmeno, #prijmeni").css("border", "2px solid red");
                            $(".btn-primary").prop("disabled", true);
                        } else {
                            $("#dupInfo").hide();
                            $("#jmeno, #prijmeni").css("border", "");
            ...
            $("#jmeno, #prijmeni").on("keyup change", kontrolaDuplicit);
