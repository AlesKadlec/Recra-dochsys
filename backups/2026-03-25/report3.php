...
// index dne pro rychlé mapování YYYY-MM-DD => sloupec
$dayIndex = [];
foreach ($days as $idx => $di) {
    $dayIndex[$di['date']] = $idx;
}
...
if (!($res = mysqli_query($conn, $sqlCounts))) {
    die("Nelze provest dotaz (counts)</body></html>");
}
...
// -----------------------------------------------------
// 1b) Denní součty: vlastní doprava vs dopravce (bus=16)
// -----------------------------------------------------
$den_vlastni  = array_fill(0, $pocetDniVMesici, 0);
$den_dopravce = array_fill(0, $pocetDniVMesici, 0);
...
if (!($res = mysqli_query($conn, $sqlDayTotals))) {
    die("Nelze provest dotaz (day totals)</body></html>");
}

while ($row = mysqli_fetch_assoc($res)) {
    $den = $row['den'];
    if (!isset($dayIndex[$den])) continue;

    $i = $dayIndex[$den];
    $den_vlastni[$i]  = (int)$row['vlastni'];
    $den_dopravce[$i] = (int)$row['dopravce'];
}
mysqli_free_result($res);
...
if (!($res = mysqli_query($conn, $sqlNeprit))) {
    die("Nelze provest dotaz (nepritomnost)</body></html>");
}
...
<!--                 <tr class='table-success fw-bold horizontal-line-hore'>
                ...
                <?php
                $suma_vlastni_mesic  = array_sum($den_vlastni);
                $suma_dopravce_mesic = array_sum($den_dopravce);
                $suma_mesic          = $suma_vlastni_mesic + $suma_dopravce_mesic; // celkem
                ?>

                <tr class='table-warning fw-bold horizontal-line-hore'>
                    <td>Denní počet zaměstnanců – vlastní doprava</td>
                    <?php foreach ($den_vlastni as $v): ?>
                        <td class='text-center text-danger'><?php echo (int)$v; ?></td>
                    <?php endforeach; ?>
                    <td class="text-center table-primary fw-bold"><?php echo (int)$suma_vlastni_mesic; ?></td>
                </tr>

                <tr class='table-warning fw-bold'>
                    <td>Denní počet zaměstnanců – dopravce</td>
                    <?php foreach ($den_dopravce as $v): ?>
                        <td class='text-center text-danger'><?php echo (int)$v; ?></td>
                    <?php endforeach; ?>
                    <td class="text-center table-primary fw-bold"><?php echo (int)$suma_dopravce_mesic; ?></td>
                </tr>

                <!-- Volitelné: necháš i "celkem" -->
                <tr class='table-success fw-bold horizontal-line-dole'>
                    <td>Denní počet zaměstnanců – celkem</td>
                    <?php for ($i=0; $i<$pocetDniVMesici; $i++): ?>
                        <td class='text-center text-danger'><?php echo (int)($den_vlastni[$i] + $den_dopravce[$i]); ?></td>
                    <?php endfor; ?>
                    <td class="text-center table-primary fw-bold"><?php echo (int)$suma_mesic; ?></td>
                </tr>
