...
function cron_na_vlastni_dopravu()
{
    ...
    if (($weekDay > 0) and ($weekDay <= 5))
    {
        ...
        {                    
            ...
            $rok = date('o');  // ISO rok (může se lišit od calendar year na přelomu roku)
            $tyden = date('W'); // číslo týdne (01–53)

            $sql = "select zamestnanci.id,zamestnanci.firma,zamestnanci.nepritomnost from plan_smen left join zamestnanci on plan_smen.jmeno = zamestnanci.id where nastup='" . $id_zastavky . "' AND rok= '" . $rok . "' AND tyden = '" . $tyden . "' AND plan_smen.smena='" . $smena . "' AND DATE(zamestnanci.vstup) <= CURDATE() AND (zamestnanci.vystup = '0000-00-00' OR DATE(zamestnanci.vystup) >= CURDATE())";
...
{    
    ...
    <div class="modal-dialog modal-dialog-centered modal-xl">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="row mt-2">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="text" class="form-control bg-primary-subtle" id="doklad" name="doklad" value="<?php echo ($id == '') ? "" : $polehodnot[0]['doklad'];?>">
                    </div>
            ...
            <div class="modal-footer">
                <button type="submit" class="btn btn-primary mb-3" id="btnUlozit">Ulož změnu</button>
                <button type="button" class="btn btn-secondary mb-3" data-bs-dismiss="modal">Zavřít</button>
            </div>
            ...
            <div class="modal-body">
                ...
                <div class="container">
                    ...
                    <div class="row mt-3">
                        ...
                        <input type="hidden" id="smena" name="smena" value="N/A">
                        <input type="hidden" id="smena2" name="smena2" value="N/A">
                    ...
                    <div class="row mt-3">
                        ...
                        <div class="col col-md-5">
                            ...
                            <select class="form-select bg-primary-subtle" id="nastup" name="nastup" aria-label="Floating label select example">
                                ...
                                <?php
                                $sql = "select nastupy.id,nastupy.zastavka from nastupy order by zastavka";
...
<!--                         <div class="col-md-2">
                    ...
                    <div class="row mt-3">
                        ...
                        <div class="col-md-4">
                            <label for="nastup">Zastávka</label>
                            <select class="form-select bg-primary-subtle" id="nastup" name="nastup" aria-label="Výběr zastávky">
                                <option value="0" <?= ($nastup == 0) ? "selected" : "" ?>>Zatím nepřiřazena</option>
                                <?php
                                $sql = "SELECT id, zastavka FROM nastupy ORDER BY zastavka";
                                $vysledek = mysqli_query($conn, $sql) or die("Nelze provést dotaz");

                                while ($radek = mysqli_fetch_assoc($vysledek)) {
                                    $selected = ($radek['id'] == $nastup) ? "selected" : "";
                                    echo "<option value='{$radek['id']}' $selected>{$radek['zastavka']}</option>";
                                }
                        ...
                        <input type="hidden" name="smena" id="smena" value="<?= htmlspecialchars($smena ?? 'N/A') ?>">
                        <input type="hidden" name="smena2" id="smena2" value="<?= htmlspecialchars($smena2 ?? 'N/A') ?>">
                    ...
                    <div class="row mt-3">
                       ...
                       <input type="hidden" name="redirect_from" value="<?php echo htmlspecialchars($_SESSION['typ_modal'] ?? '', ENT_QUOTES); ?>">
        ...
        <div class="d-grid gap-2 d-md-flex justify-content-md-center">
            <form class="row g-3" action="zamestnanci.php?typ=filtr" method="post" name="zamestnanci">
                ...
                <div class="col-md-3">
                    ...
                    <?php
                    $smeny = ['VŠE'=>'Všechny směny', 'R'=>'Ranní','O'=>'Odpolední','N'=>'Noční'];
                    ?>
