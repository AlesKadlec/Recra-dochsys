            ...
            <?php
                ...
                if ($_POST['firma'] == "ALL")
                {
                    ...
                    else
                    {
                        ...
                        elseif ($_POST['pomer'] == "Vše")
                        {
                            $sql = "select zamestnanci.id,prijmeni,jmeno,os_cislo,os_cislo_klient,rfid,telefon,adresa,firmy.firma,zastavky.zastavka,zamestnanci.smena,zamestnanci.smena2,auta.spz,zamestnanci.aktivni,nepritomnost,cilova,vstup,vystup,radneukoncen,anulace,(SELECT cas FROM dochazka WHERE zamestnanec = zamestnanci.id AND now() <= DATE_ADD(concat(datum, ' ', cas), INTERVAL 15 HOUR) ORDER BY datum DESC, cas DESC LIMIT 1) AS dochazka from zamestnanci left join firmy on zamestnanci.firma = firmy.id left join zastavky on zamestnanci.nastup = zastavky.id left join auta on zastavky.auto = auta.id where zamestnanci.firma in (" . $_SESSION["firma"] . ") and zamestnanci.aktivni='0' and anulace<>'0000-00-00' and smena " . $smena . " order by " . $sort;
                        }
                        else
                        {
                ...
                //echo $sql;
                //echo "<br>SESSION: " . $_SESSION['firma'];
                ...
                {   
                    ...
                    ?>
                        ...
                        <?php
                        if (($_POST['pomer'] == 0) or ($_POST['pomer'] == 33) or ($_POST['pomer'] == 44) or ($_POST['pomer'] == 55)  or ($_POST['pomer'] == 'Vše'))
                        {  
            ...
            <div class="collapse" id="filtry">
                <div class="d-grid gap-2 d-md-flex justify-content-md-center">
                    <form class="row g-3" action="zamestnanci.php?typ=filtr" method="post" name="zamestnanci">
                        ...
                        // Výběr firem
                        $sql = ($_SESSION["typ"] == "5")
                            ? "SELECT firma,id FROM firmy WHERE aktivni='1' ORDER BY firma"
                            : "SELECT firma,id FROM firmy WHERE aktivni='1' AND firmy.id IN (" . $_SESSION["firma"] . ") ORDER BY firma";

                        $vysledek = mysqli_query($conn, $sql) or die("Nelze provést dotaz</body></html>");
                        $firmy = mysqli_fetch_all($vysledek, MYSQLI_ASSOC);
                        mysqli_free_result($vysledek);
                        ?>

                        <div class="col-md-3">
                            <label for="firma">Výběr firmy</label>
                            <select class="form-select mt-2" id="firma" name="firma">
                                <option value="ALL">Všechny firmy</option>
                                <?php foreach($firmy as $f) : ?>
                                    <option value="<?= $f['id'] ?>" <?= (isset($_SESSION['vyberfirma']) && $_SESSION['vyberfirma'] == $f['id']) ? 'selected' : '' ?>>
                                        <?= $f['firma'] ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="col-md-3">
                            ...
                            <select class="form-select mt-2" id="pomer" name="pomer" onchange="change_pomer(this.value);">
                                <?php foreach($pomery as $val => $txt) : ?>
                                    <option value="<?= $val ?>" <?= ($val == '1') ? 'selected' : '' ?>><?= $txt ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="col-md-3">
                            ...
                            <?php
                            $smeny = ['R'=>'Ranní','O'=>'Odpolední','N'=>'Noční','NN'=>'NN','NR'=>'NR','S-R'=>'Sobota Ranní','S-O'=>'Sobota Odpolední','S-N'=>'Sobota Noční','N-R'=>'Neděle Ranní','N-O'=>'Neděle Odpolední','N-N'=>'Neděle Noční','PR'=>'Přesčas','N/A'=>'N/A','VŠE'=>'Všechno'];
                            ?>
                            <select class="form-select mt-2" id="smena" name="smena">
                                <?php foreach($smeny as $val => $txt) : ?>
                                    <option value="<?= $val ?>" <?= ($val == 'R') ? 'selected' : '' ?>><?= $txt ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="col-md-3">
                            ...
                            <?php
                            $nepritomnosti = [''=>'Přítomní','DPN'=>'DPN','OČR'=>'OČR','DOV'=>'Dovolená','ABS'=>'Absence','Vše'=>'Všechny'];
                            ?>
                            <select class="form-select mt-2" id="nepritomnost" name="nepritomnost">
                                <?php foreach($nepritomnosti as $val => $txt) : ?>
                                    <option value="<?= $val ?>" <?= ($val == '') ? 'selected' : '' ?>><?= $txt ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
