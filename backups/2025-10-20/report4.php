...
if (kontrola_prihlaseni() == "OK")
{
    if (isset($_SESSION["typ"]))
    {
        if (($_SESSION["typ"] == "5") or ($_SESSION["typ"] == "1") or ($_SESSION["typ"] == "4"))
        {
                ...
                for($i = 1; $i <= $_POST['max_den']; $i++)
                {
                    if (isset($_POST['toggle' . $i]))
                    {
                        if ($_POST['toggle' . $i] <> $_POST['lasttoggle' . $i])
                        {
                            ...
                            if (($_POST['toggle' . $i] == 'DOV') or ($_POST['toggle' . $i] == 'ABS') or ($_POST['toggle' . $i] == 'DPN') or ($_POST['toggle' . $i] == 'OČR') or ($_POST['toggle' . $i] == 'LEK') or ($_POST['toggle' . $i] == 'OABS') or ($_POST['toggle' . $i] == 'NAHV') or ($_POST['toggle' . $i] == 'NEPV'))
                            {
            ...
            //$dotaz = "SELECT DISTINCT(id) as zamestnanec,zamestnanci.cilova from zamestnanci where vstup <= '" . $start_day . "' and '" . $end_day . "' <= replace(vystup,'0000-00-00','2099-12-31') and zamestnanci.cilova='" . $cilova . "' order by zamestnanci.prijmeni";

            $dotaz = "SELECT id as zamestnanec,cilova FROM zamestnanci WHERE (nastup <= LAST_DAY('" . $start_day . "')) AND (vystup IS NULL OR vystup >= '" . $start_day . "') and cilova='" . $cilova . "' order by zamestnanci.prijmeni";
         
            //cmd.CommandText = "select count(*) from employee where date_entrance <= '" & Format(dat_1.Value, "yyyy-MM-dd") & "' and '" & Format(dat_2.Value, "yyyy-MM-dd") & "' <= replace(date_exit,'0000-00-00','2099-12-31') and group_permition in (" & permition & ")" & podminka & "order by surname"
            ...
            foreach ($num_id as $value) 
            {
                ...
                for ($x = 0; $x <= $dnu; $x++) 
                {
                    ...
                    {
                        if (($hodnota == 'DOV') or ($hodnota == 'ABS') or ($hodnota == 'DPN') or ($hodnota == 'NEPV') or ($hodnota == 'NAHV') or ($hodnota == 'LEK') or ($hodnota == 'OABS'))
                        {   ?>
