...
<head>
    ...
    <title>HLAVNÍ STRÁNKA RECRA SYSTÉMU</title>

    <!-- Poprvé nahoře v <head> nebo na začátku body -->
...
{   
    ...
    {   ?>
        ...
        {   ?>
            ...
            <script>
            window.loadModalContentNew = function(modalId, functionName, modalName) {
                console.log("Volání loadModalContentNew:", modalId, functionName, modalName);

                $.ajax({
                    url: 'funkce.php',
                    type: 'GET',
                    data: { functionName: functionName, ID: modalId },
                    success: function(response) {
                        console.log("AJAX success, response length:", response.length);

                        // Vložíme HTML modalu do stránky
                        $('#modalContent').html(response);

                        // Počkej, až se modal opravdu vloží do DOM
                        requestAnimationFrame(function() {
                            var modalEl = document.querySelector(modalName);
                            if(modalEl) {
                                console.log("Modal element nalezen:", modalEl);
                                var modal = new bootstrap.Modal(modalEl, {
                                    backdrop: 'static',  // zabrání zavření kliknutím mimo modal, můžeš upravit
                                    keyboard: false      // zabrání zavření ESC
                                });
                                modal.show();
                            } else {
                                console.error('Modal element ' + modalName + ' nenalezen.');
                            }
                        });
                    },
            ...
            <a class="btn btn-success text-center m-1" href="#" role="button"
            data-bs-toggle='modal' 
            onclick="loadModalContentNew(0,'novy_ukol','#ModalNovyUkol')">Nový úkol</a>
...
<!-- 3) UDĚL FUNKCI GLOBÁLNÍ a loadModalDoprava -->
<script>
window.loadModalDoprava = function() {
    var functionName = 'modal_doprava';
    $.ajax({
        url: 'funkce.php',
        type: 'GET',
